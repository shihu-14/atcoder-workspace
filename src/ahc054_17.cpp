...
-G周りの配置
・①＋②を探査, なければ①のみ探査.①のみの場合は, 木の周りは空きマスにする.
・①単純なジグザグ道
    ・ゴール周りは,一本道のジグザグの道を探していき, 見つかり次第終了し, その道の両脇に木を置く.
・②釣り針型の道
    ・Gの周りの木がちゃんと確認されるように①からG方向に迂回する道を探す.
    ・loop_pathで囲まれた部分について, より多くのマスが視認されているものを選択する.
        ・すべてマスが視認されていれば. 他の条件を無視しそれを採用する.
        ・そうでなければ, できるだけ多くのマスが視認されているものを採用する.
    ・Sから視認されるものを採用する.
    ・壁はすでに視認されているものとする.
    ・Gからの最初の一手をある程度許容する.
...
-メモ
    ・end_alignmentで最後の微調整をする.
        ・t_spreadが低すぎるのを防ぐ.
    ・G周りの2つのtargetを満たしている前提で, targetをより多く満たしているのを採用する.
    ・パラメータの調整
        ・op周りのやつが特に.
    seed2, 63, 37, 35
    seed46, 35, 34, 33, 27, 18
    3桁 68, 34, 41, 24, 28, 39, 50, 74, 77, 27, 17, 25. 0, 18, 69, 33 
    2, 37, 35
    多い 
    [ 3, 5, 14, 15, 16, 19, 20, 24, 26, 29, 30, 32, 40, 
     44, 47, 48, 52, 54, 55, 57, 58, 60, 62, 64, 71, 73, 
     74, 75, 79, 82, 90, 92, 93, 94, 97, 98, 99 ]
...
double t_curve = 0; // 盤面の入り組み具合を評価する
double t_spread = 500; // 到達可能なますの数を評価
double t_corner = 100;
double t_sg = 1; // スタートからゴールまでの距離を評価する
double t_dist_error = 10; // 盤面の外周にある木の本数を評価する
double t_loop = 0; // 盤面全体が行き来しやすいことを評価する
double t_tree = 100; // 木の本数を評価する
double t_edge_tree = 130; // 盤面の外周にある木の本数を評価する
double t_vis = 80;

double t_curve_rate = 0.45;
double t_spread_rate = 1;
double t_corner_rate = 0.25;
double t_tree_density = 0.48; // 盤面全体に対して設置する木の割合
double t_edge_tree_density = 0.4; // 盤面の外周に対して設置する木の割合
            ...
            for (int d1: {0, 2})
            {
                    ...
                    { // 迂回する道を探す
                        ...
                        auto dfs = [&](auto f, int x, int y) -> void
                        {
                            ...
                            if (loop_path.size()) 
                            {
                                ...
                                return;
                                // int tmp_not_visible_cnt = 0;
                                // {
                                //     queue<pii> inside = {{gx, gy}};
                                //     set<pii> used_inside; used_inside.emplace(gx, gy);
                                //     while(1)
                                //     {
                                //         vector<pii> tmp_inside;
                                //         for (auto [ix, iy]: inside)
                                //         {
                                //             rep(d, 4)
                                //             {
                                //                 int nix = ix + dx[d];
                                //                 int niy = iy + dy[d];
                                //                 if (nix < 1 || n < nix || niy < 1 || n < niy) continue;
                                //                 if (loop_path.count({nix, niy})) continue;
                                //                 if (used_inside.count({nix, niy})) continue;
                                //                 tmp_inside.emplace_back(nix, niy);
                                //                 used_inside.emplace(nix, niy);
                                //             }
                                //         }
                                //         if (tmp_inside.size() == 0) break;
                                //         inside.insert(inside.end(), rng(tmp_inside));
                                //     }
                                //     rep(i, tol+1)
                                //     {
                                //         auto [tx, ty] = tmp_stk[i];
                                //         auto it = find(rng(inside), pii{tx, ty});
                                //         if (it != inside.end())
                                //         {
                                //             inside.erase(it);
                                //         }
                                //     }
                                //     for (auto [tx, ty]: inside)
                                //     {
                                ...
                                //     }
                                // }
                                // if (tmp_not_visible_cnt == 0)
                                // {
                                //     if (res.empty() || stk_road.size() < res.size())
                                //     {
                                //         res = stk_road;
                                //         target_trees = loop_path;
                                //         not_visible_cnt = tmp_not_visible_cnt;
                                //     }
                                // }
                                // else
                                // {
                                //     if (chmin(not_visible_cnt, tmp_not_visible_cnt))
                                //     {
                                //         res = stk_road;
                                //         target_trees = loop_path;
                                //         not_visible_cnt = tmp_not_visible_cnt;
                    ...
                    int d1, int d2, bool ok=true, int curve=1, int straight=0) -> bool
                {
                    ...
                    if (t_g_dir <= curve) 
                    {
                        ...
                        //     ll sg = reachable(path_all, false);
                        //     if (sg == -INF) return false;
                        //     sub_score = {-not_visible_cnt, -(int)path_all.size()};
                        //     if (sub_best_score < sub_score)
                        //     {
                        //         sub_best_score = sub_score;
                        //         sub_best_path = path_all;
                        //         sub_best_target_trees = tmp_target_trees;
                        //     }
                        //     return false;
                        // }
                    }
...
// 盤面全体の入り組み具合を評価する関数
// {曲がり角の数/距離の総和, 到達可能なマスの数の割合, 行き止まりの割合, SからGまでの距離, ループの数, centerからの距離の誤差割合, Gまで到達可能か}
tuple<double, double, double, double, double, double, bool> board_score()
{
    int cx = sx, cy = sy;
    queue<pii> empty_q; q.swap(empty_q);
    ...
    rep(i, n+2) fill(rng(from[i]), pii{-1, -1});
    for (int i = -t_c_dir; i <= t_c_dir; i++)
    {
        for (int j = -t_c_dir; j <= t_c_dir; j++)
        {
            int nx = cx + i;
            int ny = cy + j;
            if (1 <= nx && nx <= n && 1 <= ny && ny <= n && g[nx][ny] != 'T')
            {
                q.emplace(nx, ny);
                tmp_dist[nx][ny] = 0;
            }
    ...
    rep2(i, 1, n+1)rep2(j, 1, n+1)
    {
        ...
        else if (i == 1 || i == n || j == 1 || j == n)
        {
            ...
            if (g[ni1][nj1] != 'T' && g[ni2][nj2] != 'T') global_edge_trees++;
        }
        if (tmp_dist[i][j] != INF) 
        {
            if (i == 1 || i == n || j == 1 || j == n)
            {
                dist_error += abs(tmp_dist[i][j]-(abs(i-cx)+abs(j-cy)));
                v_edge++;
            }
            turns_sum += turns[i][j];
            dists += tmp_dist[i][j];
    ...
    bool enable = tmp_dist[gx][gy] != INF && dists != 0 && v_all != 0 && v != 0 && v_edge != 0;
    if (!enable) return {-INF, -INF, -INF, -INF, 0, INF, false};
    else return {turns_sum/dists, v/v_all, v_corner/v, tmp_dist[gx][gy], e/2-v+1-f4, dist_error/v_edge, true};
}
...
{
    double res = 0;
    auto [d_curve, d_spread, d_corner, d_sg, d_loop, d_dist_error, reachable] = board_score();
    ...
    { // 盤面の入り組み具合を評価(**先にdistを計算しておくこと)
      // 到達可能なマスの数を評価  
        ...
        res += d_sg * t_sg;
        res += d_loop * t_loop;
        res -= d_dist_error * t_dist_error;
        res -= abs((double)global_trees/(n*n)-t_tree_density)*t_tree;
        res -= abs((double)global_edge_trees/(4*n-4) - t_edge_tree_density)*t_edge_tree;
    ...
    { // debug
        ...
        debug_tmp_dist_error = d_dist_error;
        debug_tmp_sg = d_sg;
...
vector<pii> alignment()
{
    ...
    while(1)
    {
        ...
        if (accept) // update
        {
            ...
            debug_dist_error = debug_tmp_dist_error;
    ...
    debug_edge_trees_rate = (double)global_edge_trees/(4*n-4);
...
int main()
{
    ...
    if (IS_LOCAL_JUDGE)
    {
        ...
        cerr << "debug_global_edge_trees_rate: " << debug_edge_trees_rate << " / " << t_edge_tree << endl;
        ...
        cerr << "debug_dist_error: " << debug_dist_error << endl;
