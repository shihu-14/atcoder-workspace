...
void solve()
{
    ll n; cin >> n;
    auto divs = divisor(n);
    vector<mint> fact(D, 1);
    rep(i, D-1) fact[i+1] = fact[i]*(i+1);
    // printv(divs);
    mint ans;
    // vector<ll> tmp;
    auto f = [&](auto f, int i, ll x, ll sum, ll c) -> void
    {
        if (x == 1)
        {
            ans += sum*fact[c];
        ...
        {
            if (x/divs[j] < divs[j]) break;

            if (x%divs[j]) continue;
            // tmp.emplace_back(divs[j]);
            f(f, j+1, x/divs[j], sum+divs[j], c+1);
            // tmp.pop_back();
        }
        if (x >= divs[i])
        {
            // tmp.emplace_back(x);
            f(f, divs.size(), 1, sum+x, c+1);
            // tmp.pop_back();
        }
    };
    f(f, 1, n, 0, 0);
