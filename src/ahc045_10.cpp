...
int main(){
    ...
    rep(qi, q){
        ...
        rep(t, sz){
            ...
            int d2 = abs(i-ni) + abs(j-nj);
            pull_gravity(vs[0], u, abs(d2-d));
            widx[u] = p[u].fi/w; widy[u] = p[u].se/w;
            if (i == widx[u] && j == widy[u]) continue;
            div_groups[i][j].erase(find(rng(div_groups[i][j]), u));
            div_groups[widx[u]][widy[u]].emplace_back(u);
        }
    }
