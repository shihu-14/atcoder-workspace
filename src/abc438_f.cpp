...
void solve()
{
    ...
    rep2(i, 1, n)
    {   
        if (lca.is_on_pass(u, v, i))
        {
        }
        else if (lca.is_on_pass(i, v, u))
        {
            u = i;
        }
        else if (lca.is_on_pass(i, u, v))
        {
            v = i;
        }
        else
    ...
    for (auto [u, v]: paths)
    {
        ...
        if (u == w)
        {
            ans += (ll)lca.sz[v]*(n - lca.sz[lca.get_kth_ancestor(v, lca.get_length(u, v)-1)]);
        }
        else if (v == w)
        {
            ans += (ll)lca.sz[u]*(n - lca.sz[lca.get_kth_ancestor(u, lca.get_length(u, v)-1)]);
        }
        else
        {
            ans += (ll)lca.sz[u]*lca.sz[v];
        }
    }
    cout << ans << endl;
}
