...
void pull_gravity(vector<int> &vs, set<pii> &query_edges){
    int sz = vs.size();
    dsu uf(sz);
    vector<T3> edges;
    ...
    for (auto [w2, i, j]: edges){
        ...
        if (!query_edges.count({vs[i], vs[j]})) longer.emplace_back(vs[i], vs[j]);
        if (cnt == sz-1) break;
    }
    for (auto [u, v]: query_edges) shoter.emplace_back(u, v);
    double rate = L;
    // for (auto [u, v]: shoter){
    //     auto [x1, y1] = p[u];
    //     auto [x2, y2] = p[v];
    //     auto [lx1, rx1, ly1, ry1] = bounding_box[u];
    //     auto [lx2, rx2, ly2, ry2] = bounding_box[v];
    //     double nx1 = x1 + (x2-x1)*rate/100.0;
    //     double ny1 = y1 + (y2-y1)*rate/100.0;
    //     double nx2 = x2 + (x1-x2)*rate/100.0;
    //     double ny2 = y2 + (y1-y2)*rate/100.0;
