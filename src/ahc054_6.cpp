...
vector<pii> initial_alignment(){
    ...
    vector<vector<char>> onpath(n+2, vector<char>(n+2, 0));
    if (dist[gx][gy] != INF){
        int x=gx, y=gy;
        while(!(x==sx && y==sy)){
            onpath[x][y]=1;
    ...
    // place trees on parity cells not on the path
    for(int i=1;i<=n;i++) for(int j=1;j<=n;j++){
...
// 盤面の入り組み具合を評価する関数
tuple<double, double, bool> curve_score(int sx=1, int sy=n/2+1)
{
    queue<pii> q;
    q.emplace(sx, sy);
...
double calculate(vector<pii> &trees)
{
    ...
    { // 盤面の入り組み具合を評価(**先にdistを計算しておくこと)
      // 到達可能なマスの数を評価  
        auto [d_curve, d_spread, reachable] = curve_score();
        res += d_curve * t_curve;
    ...
    if (false){ // ゴールの周りの半径d周の堅固さ
        res += guard_score(trees) * t_guard;
    }
    return res;
}
