...
int main(){
    ...
    for (auto [type, x]: query){
        if (type == 1){
            ...
            if (seg.get(j) == 1 && j && abs(cc[j]-cc[j-1]) <= k){
                int l = seg.min_left<f>(j);
                ans += j-l;
                ans++;
                cout << ans << '\n';
            }
            else if (seg.get(j) == 1 && j+1 < m && abs(cc[j]-cc[j+1]) <= k){
                int r = seg.max_right<f>(j+1);
                ans += r-j-1;
                ans++;
                cout << ans << '\n';
            }
