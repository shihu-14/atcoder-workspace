...
void end_alignment(vector<pii> &res)
{
    auto dijkstra = [&]()
    {
        queue<pii> empty_q; q.swap(empty_q);
    ...
    for (int i = -t_e_dir; i <= t_e_dir; i++)
    {
        for (int j = -t_e_dir; j <= t_e_dir; j++)
        {
            int x = gx + i;
            int y = gy + j;
            if (x < 1 || n < x || y < 1 || n < y) continue;
            if (g[x][y] != 'T') continue;
            auto it = find(rng(res), pii{x, y});
            ...
            bool ok = false;
            {
                rep(d, 2)
                {
                    ...
                    if (dist[nx1][ny1] != INF && dist[nx2][ny2] != INF 
                        && abs(dist[nx1][ny1]-dist[nx2][ny2]) > t_e_diff)
                    {
                        ok = true;
                        break;
                    }
                ...
                int nx2 = x, ny2 = y+1;
                int nx3 = x+1, ny3 = y+1;
                if (dist[nx1][ny1] != INF && dist[nx2][ny2] != INF && g[nx3][ny3] == 'T'
                    && abs(dist[nx1][ny1]-dist[nx2][ny2]) > t_e_diff)
                {
                    ok = true;
                }
