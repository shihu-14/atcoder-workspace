...
vector<T> simulator(vector<pii> &box, int &final_score, int BW){
    int n = box.size();
    vector<T> res;
    vector<Node> put_box;
...
int main(){
    ...
    while(t--){
        if (t <= tt/8) break;
        int dummy = 0, BW = 10;
        auto ans = simulator(box, dummy, BW);
        output(ans);
    }
    priority_queue<pair<int, vector<T>>, 
            vector<pair<int, vector<T>>>, 
            greater<pair<int, vector<T>>>> pq;
    while(1){
        int tmp_score = 0;
        vector<T> res = simulator(box, tmp_score, chBW());
        pq.emplace(tmp_score, res);
        if (pq.size() > tt/8) pq.pop();
        auto end = std::chrono::high_resolution_clock::now();
        auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
        if (elapsed.count() > 2900) break;
    }
    while(t--){
        auto [_, ans] = pq.top(); pq.pop();
        output(ans);
    }
    return 0;
}