...
vector<vector<int>> a_r = {{20, 0, 0, 20, 40}, {30, 0, 0, 30, 40}, 
    {30, 0, 0, 30, 40}, {15, 0, 0, 15, 70}};
...
double t_tree_density = 0.35; // 盤面全体に対して設置する木の割合
double t_vis_rate = 4.8; // 全空きマスうち, Sから到達可能なマスの割合

ll t_s_dir = 20; // 初期配置の間隔
ll t_g_edir = 4; // ゴール周りには木を置かない
ll t_g_dir = 3; // ゴール周りの堅固さを評価する際の範囲
ll t_g_straight = 1; // ゴール周りの堅固さを評価する際の直線距離
...
vector<pii> goal_alignment(){
    ...
    for (int d2: {2, 1, 3, 0})
    {
        ...
        for (int d1: {0, 2})
        {
            ...
            auto return_alignment = [&]() -> vector<pii>
            {
                auto trees = apply_path(tmp_stk); // pathの周りに木を置いている.
                vector<pii> q_trees;
                { // 木を連結成分を作る
                    set<pii> used; // usedを管理
                    for (auto [x, y]: tmp_stk)
                    {
                        rep(d, 8)
                        {
                            int nx = x + dxx[d], ny = y + dyy[d];
                            if (nx < 1 || n < nx || ny < 1 || n < ny) continue;
                            if (g[nx][ny] == 'T' && !used.count({nx, ny}))
                            {
                ...
                vector<pii> res;
                { // 道を探す
                    int ngx = gx, ngy = gy; // 道の終点
                    while(1) // 道の終点を決める
                    {
                        ngx += dx[(d2+2)%4], ngy += dy[(d2+2)%4];
                        if (ngx < 1 || n < ngx || ngy < 1 || n < ngy)
                        {
                            revert_path(trees);
                            return res;
                        }
                    ...
                    set<pii> road;
                    for (auto [x, y]: q_trees) // 木の周囲の空きマスをroadに入れる
                    {
                        rep(d, 8)
                        {
                            int nx = x + dxx[d];
                    ...
                    auto dfs = [&](auto f, int x, int y) -> void
                    {
                        ...
                        if (x == ngx && y == ngy)
                        {
                            if (res.empty() || res.size() < stk_road.size())
                            {
                                res = stk_road;
                            }
                            return;
                    ...
                    auto [sx, sy] = tmp_stk.back();
                    // cout << sx << " " << sy << endl;
                    // cout << ngx << " " << ngy << endl;
                    // for (auto [x, y]: q_trees) cout << x << " " << y << endl;
                    // for (auto [x, y]: road)
                    // {
                    //     g[x][y] = 'R';
                    // }
                ...
                int d1, int d2, bool ok=true, int curve=1, int straight=0) -> bool
            {
                ...
                if (t_g_dir <= curve) 
                {
                    auto add_path = return_alignment();
                    if (add_path.size() == 0) return false;
                    auto all_path = tmp_stk;
                    all_path.insert(all_path.end(), rng(add_path));
                    ll sg = reachable(all_path, false);
...
vector<pii> alignment()
{
    ...
    while(1)
    {
        ...
        else if (op_kind == 1 || op_kind == 2 || op_kind == 3)
        {
            for (auto [x, y]: tmp_del)
            {
                g[x][y] = 'T';
