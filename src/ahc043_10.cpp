...
// now_money, now_turn, per_earn, sta_x, sta_y, near_x, near_y, vector<T3> ans
using PQ = tuple<int, vector<pii>, vector<T3>, int, int>;
...
set<pii> two_station(vector<vector<int>> &earnlist, int n, int K, int T){
    ...
    set<T3> init_list = {{max_money, max_p1, max_p2}};
    {
        rep(x1, n)rep(y1, n)rep(x2, n)rep(y2, n){
            int d = abs(x1-x2)+abs(y1-y2);
            if (d <= 4) continue;
            ...
            if (money < 0) continue;
            money += (earnlist[x1*n+y1][x2*n+y2]+earnlist[x2*n+y2][x1*n+y1])*(T-turn);
            if (money > max_money/2){
                init_list.emplace(money, x1*n+y1, x2*n+y2);
                if (init_list.size() > BeamWidth){
...
int main(){
    ...
    vector<pii> init_list;
    int max_money = K;
    vector<T3> final_ans(T, -1);
    vector<vector<int>> board(n, vector<int>(n));
    vector<pii> stations;
    vector<T3> ans;
    int now_money = K, now_turn = 0, per_earn = 0;
    auto que = two_station(earnlist, n, K, T);
    {
        auto [mx, cand] = 
        tie(init_x1, init_y1, init_x2, init_y2) = mx;
        init_list = cand;
