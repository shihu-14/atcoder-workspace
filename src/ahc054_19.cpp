...
double t_tree_density = 0.43; // 盤面全体に対して設置する木の割合
...
vector<pii> initial_alignment()
{
    ...
    vector<pii> path;
    set<pii> used;
    auto dfs = [&](auto f, int x=sx, int y=sy, int right=0) -> bool
    {
        if (x == n) return true;
        vector<int> d_list;
        if (right == 0)
        {
            if (sy < gy) d_list = {2, 3, 1};
            else d_list = {2, 1, 3};
        }
        else if (right > 0) d_list = {3, 2, 1};
        else d_list = {1, 2, 3};

        for (int d: d_list)
        {
            int nx = x+dx[d], ny = y+dy[d];
            if (g[nx][ny] == 'T' || used.count({nx, ny})) continue;
            bool ok = true;
            rep(td, 4)
            {
                int tx = nx + dx[td], ty = ny + dy[td];
                if (tx == x && ty == y) continue;
                if (g[tx][ty] == 'R' || g[tx][ty] == 'E' || g[tx][ty] == 'G' || used.count({tx, ty}))
                {
                    ok = false;
                    break;
                }
            ...
            used.emplace(nx, ny);
            if (f(f, nx, ny, right+(d == 1 ? 1: (d == 3 ? -1: 0))))
                return true;
            path.pop_back();
            used.erase({nx, ny});
        }
    ...
    set<pii> used_path; 
    for (auto [x, y]: path) used_path.emplace(x, y);
    bool right = false;
    vector<pii> res;
    { // 下の抜け道を探す
        if (abs(sx-gx) > n/2) reverse(rng(path)); 
        for (auto [x, y]: path)
        {
            if (g[x][y] != 'S') g[x][y] = 'P';
            bool flag = false; // 2マス先に空きマスがあるか
            int hole_d = -1;
            rep(d, 4)
            {
                bool ok = true;
                rep(i, 2)
                {
                    int nx = x + (i+1)*dx[d], ny = y + (i+1)*dy[d];
                    if (!(1 <= nx && nx <= n && 1 <= ny && ny <= n) || g[nx][ny] != '.' || used_path.count({nx, ny}))
                    {
                        ok = false;
                        break;
                    }
                    rep(td, 4)
                    {
                        int tx = nx + dx[td], ty = ny + dy[td];
                        if (tx == x && ty == y) continue;
                        if (g[tx][ty] == 'R' || g[tx][ty] == 'E' || g[tx][ty] == 'G' || used_path.count({tx, ty}))
                        {
                            ok = false;
                            break;
                        }
                ...
                int nx2 = x + 2*dx[d], ny2 = y + 2*dy[d];
                flag = true;
                hole_d = d;
                if (d == 1 || d == 3) right = (d == 1);
                else
                {
                    int tx = nx1, ty = ny1;
                    while(ty <= n)
            ...
            if (flag)
            {
                rep(d, 4)
                {
                    if (d == hole_d) continue;
                    int nx = x + dx[d], ny = y + dy[d];
                    if (g[nx][ny] != 'T' && !used_path.count({nx, ny}))
                    {
                        g[nx][ny] = 'T';
    ...
    { // 上の抜け道を探す
        ...
        for (auto [x, y]: path)
        {
            if (g[x][y] == 'P' || g[x][y] == 'S') break;
            if (g[x][y] != 'S') g[x][y] = 'P';
            if (!flag)
            {
                rep(d, 4)
                {
                    if (d == (right ? 1: 3)) continue;
                    if (d == 1 || d == 3)
                    {
                        int nx1 = x + dx[d], ny1 = y + dy[d];
                        int nx2 = x + 2*dx[d], ny2 = y + 2*dy[d];
                        if (g[nx1][ny1] == '.' && (1 <= nx2 && nx2 <= n && 1 <= ny2 && ny2 <= n) &&
                            g[nx2][ny2] == '.' && !used_path.count({nx1, ny1}) && !used_path.count({nx2, ny2}))
                        {
                            flag = true;
                            hole_d = d;
                            g[nx1][ny1] = 'P';
                    ...
                    else 
                    {
                        ...
                        int nx2 = x + 2*dx[d], ny2 = y + 2*dy[d];
                        if (!(g[nx1][ny1] == '.' && (1 <= nx2 && nx2 <= n && 1 <= ny2 && ny2 <= n) &&
                            g[nx2][ny2] == '.' && !used_path.count({nx1, ny1}) && !used_path.count({nx2, ny2}))) continue;
                        int tx = x + dx[d], ty = y + dy[d];
                        bool tmp_right = false;
                        while(ty <= n)
                        {
                            ty += dy[1];
                            if (used_path.count({tx, ty}))
...
vector<pii> goal_alignment()
{
    ...
    // main process
    vector<pii> res, best_path;
    set<pii> best_target_trees;
    pll best_score = {-INF, -INF};
