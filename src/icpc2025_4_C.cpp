...
struct Eratosthenes{
    ...
    // 素因数分解のリストから約数のリストを作る。O(約数の個数)
    void divisor3(vector<pair<ll, int>> factor, ll a, ll np, ll &ans){
        vector<ll> res;
        auto dfs = [&](auto f, int i, ll now) -> void{
            if (i == factor.size()){
                // res.emplace_back(now);
                chmin(ans, a+now+(np/a/now));
                return;
            }
            auto [p, e] = factor[i];
            ll val = 1;
...
int main(){
    ...
    while(1){
        ll p; cin >> p;
        if (p == 0) return 0;
        auto factor = es.factorize1(p);
        if (factor.empty()){
            cout << p+2 << "\n";
            continue;
        }
        ll ans = LINF;
        auto dfs = [&](auto f, int i, ll sa, ll sb, ll sc) -> void{
            if (i == factor.size()){
                chmin(ans, sa+sb+sc);
                return;
            }
            ...
            for (int a = 0; a <= e; a++){
                ...
                for (int b = 0; b <= e-a; b++){
                    ll nc = 1;
                    for (int c = 0; c < e-a-b; c++) nc *= p;
                    f(f, i+1, sa*na, sb*nb, sc*nc);
                    nb *= p;
                }
                na *= p;
            }
