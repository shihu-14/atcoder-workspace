...
int main(){
    ...
    bit_Matrix a(n, n+1);
    vector<int> query; 
    rep(i, k) query.emplace_back(i);
    vector<vector<int>> query_ans(n);
    rep(i, k+1){
        cout << "? ";
        rep(j, k) cout << query[j]+1 << " ";
        cout << endl;
        int x; cin >> x; a[i][n] = x;
        query_ans[i] = query;
        query_ans[i].emplace_back(x);
        rep(j, k){
            a[i][query[j]] = 1;
        }
        query.emplace_back((query.back()+1)%(k+1));
    ...
    rep(i, n){
        int res = 0;
        rep(j, k) res ^= ans[query_ans[i][j]];
        if (res != query_ans[i][k]){
            assert(0);
        }
    }
    cout << "!";
