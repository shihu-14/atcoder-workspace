...
vector<vector<int>> pos(er.prime.size());
...
void solve()
{
    ...
    rep(i, n)
    {
        ...
        for (auto [p, e] : pf)
        {
            int id = lower_bound(rng(er.prime), p) - er.prime.begin();
            if (pos[id].size() == 0)
            {
                tmp_p.emplace_back(id);
            }
            pos[id].emplace_back(e);
        }
    }
    ...
    rep(i, n)
    {
        ...
        {
            int id = lower_bound(rng(er.prime), p) - er.prime.begin();
            int last = lower_bound(rng(pos[id]), e) - pos[id].begin();
            if (last != pos[id].size()-1) continue;
            if (last >= 1)
            {
                int diff = pos[id].back() - pos[id][last-1];
                x *= mint(er.prime[id]).pow(diff);
            }
            else
            {
                int diff = pos[id].back() - 0;
                x *= mint(er.prime[id]).pow(diff);
            }
        }
        mint ans = L / x;
