...
vector<pair<char, int>> calc(vector<T> op_que, vector<vector<int>> board){
    ...
    for (auto [r, k, d]: op_que){
        ...
        int cost1 = abs(sx-bx)+abs(sy-by); 
        cost1 *= 3;
        int cost2 = INF;
        // calc cost2
        if (d == 0 || d == 2){
            rep(i, n) if (board[i][k] == -1){
                if (board[i][0] != -1 || board[i][n-1] != -1) cost2 += 2;
                else{
                    cost2 = INF; 
                    break;
                }
            ...
            rep(j, n) if (board[k][j] == -1){
                if (board[0][j] != -1 || board[n-1][j] != -1) cost2 += 2;
                else{
                    cost2 = INF;
                    break;
                }
        ...
        // finish calc
        cost2 += n;
        if (cost2 != INF && cnt_oni >= 3){
            vector<pii> tmp_op;
            if (d == 0 || d == 2){
                rep(i, n) if (board[i][k] == -1){
                    if (board[i][0] != -1){
                        tmp_op.emplace_back(3, i);
                    }
                    else{
                        tmp_op.emplace_back(1, i);
                    }
                }
            }
            else{
                rep(j, n) if (board[k][j] == -1){
                    if (board[0][j] != -1){
                        tmp_op.emplace_back(0, j);
                    }
                    else{
                        tmp_op.emplace_back(2, j);
                    }
                }
            }
            for (auto [d, p]: tmp_op){
                res.emplace_back(dc[d], p);
            }
            if (d == 0 || d == 2){
                rep(i, n){
                    if (board[i][k] == 1) board[i][k] = 0;
                    res.emplace_back(dc[0], k);
                }
            }
            else{
                rep(i, n){
                    if (board[k][i] == 1) board[k][i] = 0;
                    res.emplace_back(dc[1], k);
                }
            }
            for (auto [d, p]: tmp_op){
                if (d < 2) d += 2;
