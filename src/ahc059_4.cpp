...
void hill_climbing() {
    ...
    // --- 焼きなましパラメータ設定 (指数冷却用) ---
    double start_temp = 100.0; // 変更: 高温スタート
    double end_temp = 0.1;    // 変更: 十分低い値
    ...
    rep(p, 3) {
        ...
        // Regraft
        t = phase_stats[p].regraft.try_count;
        u = phase_stats[p].regraft.update_count;
