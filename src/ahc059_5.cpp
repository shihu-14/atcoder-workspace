...
// --- 初期解構築のバリエーション ---
// mode 0: 2点間距離 降順 (既存・基本最強)
// mode 1: ランダム
// mode 2: 2点間距離 昇順
// mode 3: 原点(0,0)からの距離(2点の和) 降順 (遠くから)
// mode 4: 原点(0,0)からの距離(2点の和) 昇順 (近くから)
// mode 5: 行番号(r)の和 昇順 (上から下へ)
void build_initial_solution(int mode) {
    ...
    rep(i, M) {
        ...
        if (mode == 0 || mode == 2) {
            // 2点間距離
            val = dist(card_pos[i][0], card_pos[i][1]);
        } else if (mode == 3 || mode == 4) {
            // 原点からのマンハッタン距離の和 (位置情報)
            val = (card_pos[i][0].r + card_pos[i][0].c) + (card_pos[i][1].r + card_pos[i][1].c);
        } else if (mode == 5) {
            // 行番号の和 (スキャン順)
            val = card_pos[i][0].r + card_pos[i][1].r;
        } else {
            // mode 1 (Random) は値関係なし
            val = 0; 
        }
    ...
    if (mode == 0 || mode == 3) {
        sort(rng(pairs_by_val), greater<pair<int, int>>()); // 降順
    } else if (mode == 2 || mode == 4 || mode == 5) {
        sort(rng(pairs_by_val)); // 昇順
    } else {
        shuffle(rng(pairs_by_val), engine); // ランダム
    }

    // 以下、Cheapest Insertion のロジックは共通
    root_id = M;
...
void hill_climbing(chrono::steady_clock::time_point start_time) {
    ...
    // パラメータ (start_temp=100.0, end_temp=0.1)
    double start_temp = 400.0; 
    double end_temp = 0.1;     
    double time_limit = 1975.0; 

    int iter = 0;
    
    // Phase thresholds (Regraft, Flip)
    ...
    while (true) {
        ...
        if (type_rnd < prob_regraft) {
            ...
            Point t_end = get_end_pos(target); // Note: tree info updated by Flip is not here, but flip is not changed
    ...
    rep(p, 3) {
        ...
        long long t = phase_stats[p].regraft.try_count;
        long long u = phase_stats[p].regraft.update_count;
        double r = (t > 0) ? (double)u / t * 100.0 : 0.0;
        cerr << op_names[0] << ": " << setw(8) << t << " / " << setw(8) << u << " (" << fixed << setprecision(2) << setw(6) << r << "%)" << endl;
        phase_try += t; phase_upd += u;

        t = phase_stats[p].flip.try_count;
        u = phase_stats[p].flip.update_count;
        r = (t > 0) ? (double)u / t * 100.0 : 0.0;
        cerr << op_names[1] << ": " << setw(8) << t << " / " << setw(8) << u << " (" << fixed << setprecision(2) << setw(6) << r << "%)" << endl;
        phase_try += t; phase_upd += u;

        t = phase_stats[p].swap.try_count;
        u = phase_stats[p].swap.update_count;
        r = (t > 0) ? (double)u / t * 100.0 : 0.0;
...
void solve()
{
    ...
    // --- 1. 多点スタート (Multi-Start Strategy) ---
    // 最初の250msを使って、複数の初期解を生成・評価し、ベストなものを採用する
    ...
    // 定義済みのheuristic mode (0~5) を一通り試す
    ...
    // 残り時間はランダム(mode 1)で試行回数を稼ぐ
    while (true) {
        auto now = chrono::steady_clock::now();
        double elapsed = chrono::duration_cast<chrono::milliseconds>(now - start_time).count();
        if (elapsed > multi_start_limit_ms) break;

        build_initial_solution(1); // Random
        int score = calc_total_score();
        if (score < best_initial_score) {
            best_initial_score = score;
            best_initial_tree = tree;
