...
struct Tree{
    ...
    void move(int t){
        ...
        op_histroy.emplace_back(string(1, dc[t]));
    }
    void rotate(int c){
...
int main(){
    ...
    int hold_id = V/2, turn_min = INF;
    vector<string> ans;
    rep2(hold, 1, V){
        int turn_tmp = 0;
        tree.history_clear();
        while(!tree.isfinish()){
            while(!tree.toput(hold)){
                rep2(i, 1, n){
                    if (tree.toput(hold)) break;
                    int rx = tree.tree[0].x, ry = tree.tree[0].y;
                    int nx = board.query[m-board.res_ball].fi, ny = board.query[m-board.res_ball].se;
