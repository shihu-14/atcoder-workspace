...
const int op_rate = 500;
int tol_central_dist;
const vector<int> div_size = {20, 15, 10, 7, 4};
const vector<double> coef = {1, 5, 0.1};
const vector<int> edge_coef = {0, -50, -300, -1000, -2000, -3000, -5000, -10000, -20000, -30000, -40000, -50000};
...
int main(){
    ...
    cerr << "init_score " << score << endl;
    double min_score = LINF;
    int times = 0, update = 0, accept = 0;
    random_device rd; mt19937 gen(rd()); 
    uniform_int_distribution<int> dist(0, n-1);
    uniform_real_distribution<> real_dist(0.0, 1.0);
    int op = 0;
    double start_temp = 1e4, end_temp = 1e-8;
    while(1){
        auto end = std::chrono::high_resolution_clock::now();
        auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
        if (elapsed.count() > 1900) break;
        double temp = (start_temp-end_temp)*(500.0-(double)elapsed.count())/500.0 + end_temp;
        op++;
        times++;
        ...
        double delta = min_score-score;
        double prob = exp(delta*200.0/temp);
        // cerr << temp << " " << delta << " " << prob << endl;
        // temp *= 0.60;
        // printf("%016f, %016f, %016f\n", temp, delta, prob);
        if (delta > 0){
            min_score = score;
            update++;
            continue;
        }
        else if (prob < 0.9999 && real_dist(gen) < prob){
            min_score = score;
            accept++;
            continue;
        }
    ...
    { // debug info
        cerr << "final_score: " << min_score << endl;
