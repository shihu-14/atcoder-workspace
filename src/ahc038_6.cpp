...
struct Tree{
    ...
    vector<int> res_stk, pulling_stk;
    ...
    void pull(){
        ...
        rep2(i, 1, tn){ // ノードの位置が盤面に収まっていて、今ボールを掴んでいないかつ、そこにボールがある場合、pullする。
            ...
            board->res_ball--;
            pulling_ball++;
            if (res_stk.back() == board->id[{x, y}]) cash_clear();
        ...
        rep2(i, 1, tn){
            if(tree[i].caught){ // ボールを掴んでいるノードのうち、目的地が今のノードの位置と一致するものはputする。
                ...
                if (x == nx && y == ny){
                    ...
                    board->put_ball++;
                    pulling_ball--;
                    if (pulling_stk.back() == i) cash_clear();
    ...
    double calc_f(int op1, int op2, bool ispull){
        ...
        if (ispull){ // ボールを引き上げる時において、
            int i = res_stk.back(), j = i%tn;
            for(; cord[j].caught; j = (j+1)%tn);
            int x = cord[j].x, y = cord[j].y;
            auto [nx, ny] = board->query[i];
            res += sqrt((x-nx)*(x-nx) + (y-ny)*(y-ny));
        }
        else{ // ボールを置く時において、
            int i = pulling_stk.back(); 
