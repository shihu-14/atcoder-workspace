...
-近傍操作
・木をn本追加
    ・上下左右に空きますが多いほど、選ばれやすくなる.
・木をn本削除
    ・4マス見た時に不要な木がある場合削除する.
    ・木の両端の空きマスを見た時にdistの差分が大きい木を削除.
・木をn本移動(隣接マスに移動)
...
vector<vector<int>> spacing_count;
double vis_score;
ll trees_count;
set<T3> cand_add, cand_del;
    ...
    for (int d1: {2, 1, 3, 0})
    {
        for (int d2: {0, 2})
        {
            ...
            auto dfs = [&](auto f, int x, int y, int p1x, int p1y, 
                int d1, int d2, bool ok, int curve, int straight) -> bool
            {
                if (straight > guard) return false;
                if (curve >= dir) 
                {
                    int r = reachable(stk);
                ...
                for (auto d: ds)
                {
                    ...
                    if (g[nx][ny] == '.') 
                    {
                        ...
                        if (f(f, nx, ny, x, y, nd1, nd2, nok, curve+nok, straight+!nok))
                        {
                            return true;
                        }
                        stk.pop_back();
...
{
    double delta_score = 0, delta_count = add_trees.size() - del_trees.size();
    auto calc_delta = [&](bool is_add=true) -> double
    {
        ...
        for (auto [x, y]: trees)
        {
            if (rows[x].size())
            {
                ...
                if (it != rows[x].begin())
                {
                    res += abs(y - *(it-1) - 1)*minus;
                }
                if (it != rows[x].end())
                {
                    res += abs(*it - 1 - y)*minus;
                }
            }
            if (cols[y].size())
            {
                ...
                if (it != cols[y].begin())
                {
                    res += abs(x - *(it-1) - 1)*minus;
                }
                if (it != cols[y].end())
                {
                    res += abs(*it - 1 - x)*minus;
                }
            }
        }
        return res;
    ...
    double res = (trees_count*delta_score-vis_score*delta_count)/(trees_count*(trees_count+delta_count));
    return res;
}


// 盤面全体の入り組み具合を評価する関数
// {盤面の曲がりくねり具合, 到達可能なマスの数, スタートからゴールまでの距離, 閉路の数, ゴールに到達可能か}
tuple<double, double, double, double, bool> curve_score()
{
    ...
    rep2(i, 1, n+1)rep2(j, 1, n+1)
    {
        if (dist[i][j] != INF) 
        {
            ...
            rep(d1, 2)rep(d2, 2)
            {
                if (dist[i+d1][j+d2] == INF) loop = false;
            }
            if (loop) f4++;
            rep(d, 4)
            {
    ...
    if (dists) res /= dists;
    return {res, v, dist[sx][sy], e/2-v+1-f4, dist[sx][sy] != INF && dists != 0};
}
    ...
    while(1)
    {
        ...
        if (op_kind == 0 && cand_add.size())
        {
            rep(i, cand_add.size())
            {
                ...
                auto [x, y] = *cand_add.rbegin();
                g[x][y] = 'T';
                cand_del.emplace(x, y);
...
int main()
{
    ...
    if (IS_LOCAL_JUDGE)
    {
        ...
        cerr << "debug_first_accept_rate: " << debug_first_times << "%" << endl;
        cerr << "debug_middle_accept_rate: " <<  debug_middle_times << "%" << endl;
        cerr << "debug_last_accept_rate: " << debug_last_times << "%" << endl;   
