...
void solve()
{
    ...
    auto f = [&](auto f, int l, int r) -> void
    {
        ...
        vector<mint> a(mid-l), b(r-l);
        rep(i, mid-l) a[i] = dp[i+l];
        rep(i, r-l) b[i] = dice[i];
        auto c = convolution(a, b);

        rep2(i, mid, r) dp[i] += c[i-l];
        f(f, mid, r);
    };

    f(f, 0, n);

    rep(i, n) dice[i+1] += dice[i];
    
    mint ans;
    rep(i, n)
    {
        ans += dp[i]*(dice[n]-dice[n-i-1]);
    }
    cout << ans.val() << "\n";
}
