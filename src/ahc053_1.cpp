...
void solve(){
    ...
    auto idx_anchor = [&](ll j)->ll{ 
        return j; 
    };
    auto idx_piece  = [&](ll j, ll k)->ll{ 
        return sets + j*9 + k; 
    };

    rep(j, sets){
        if(idx_anchor(j) < N) A[idx_anchor(j)] = min<ull>(L, U);
        rep(k, 9){
            __int128 piece128 = ( __int128)d << k;
            ull piece = (piece128 > ( __int128)U ? U : (ull)piece128);
            ll id = idx_piece(j,k);
            if(id < N) A[id] = max<ull>(1ULL, min<ull>(piece, U));
        }
    }
    ...
    rep(j, sets){
        ...
        if(Bj >= L){
            ...
            if(0 <= ia && ia < N){
                X[ia] = (int)(j+1);
                S = (unsigned __int128)L;
            }
        }
        for(ll k=8;k>=0;--k){
            ll id = idx_piece(j,k);
            if(!(0 <= id && id < N)) continue;
            ull piece = A[id];
            if(S + (unsigned __int128)piece <= (unsigned __int128)Bj){
                X[id] = (int)(j+1);
                S += (unsigned __int128)piece;
            }
        }
    }
