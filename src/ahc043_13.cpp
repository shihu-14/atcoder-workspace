...
void construct(vector<vector<int>> &board, vector<T3> &ans, vector<pii> &op, int &now_money, int &now_turn, int &per_earn, int new_per_earn){
    ...
    if (now_money < 5000){ // 駅の建設のための資金稼ぎ
        if (per_earn == 0){
            cerr << "moeney is not enough" << endl;
            return;
        }
...
int main(){
    ...
    int times = 0, update = 0, alpha = 7, beta = 300, gamma = 15, del_rate = 47;
    ...
    while(1){
        ...
        if (elapsed.count() > 2940) break;
        // else if (elapsed.count() > 2000) alpha = 43, beta = 97, gamma = 10;
        // else if (elapsed.count() > 1000) alpha = 17, beta = 7, gamma = 25;
        times++;
        auto pre_que = que;
