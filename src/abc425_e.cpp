...
int main()
{
    ...
    cin >> t >> m;
    modint::set_mod(m);
    vector<vector<int>> facts(D+1, vector<int>(D+1));
    Eratosthenes er(D);
    rep2(i, 1, D+1)
    {
        auto vs = er.factorize1(i);
        for (auto [p, e] : vs)
        {
            facts[i][p] = e;
        }
    ...
    rep(i, D)
    {
        rep(j, D+1) facts[i+1][j] += facts[i][j];
    }
    while(t--)
    {
        int n; cin >> n;
        ...
        mint ans = 1;
        rep(i, D+1) ans *= mint(i).pow(fact[i]);
        cout << ans.val() << "\n";
    }
    return 0;
}