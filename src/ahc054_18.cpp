...
vector<pii> end_alignment()
{
    auto start = chrono::steady_clock::now();
    
    vector<pair<int, pii>> cand_add;
    rep2(i, 1, n+1)rep2(j, 1, n+1)
    {
        ...
        cand_add.emplace_back(r+c+1, pii{i, j});
    }
    sort(rrng(cand_add));
    auto reachable = [&]() -> bool
    {
    ...
    rep(i, cand_add.size())
    {
        ...
        if (cand_add[i].fi < 6) break;
        auto [x, y] = cand_add[i].se;
        g[x][y] = 'T';
        if (reachable())
        {
...
vector<pii> alignment()
{
    ...
    auto res_e = end_alignment();
    res.insert(res.end(), rng(res_g));
    res.insert(res.end(), rng(res_e));
