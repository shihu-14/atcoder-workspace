...
using PQ = tuple<int, vector<vector<int>>, vector<pii>, vector<T3>, int, int>;
...
tuple<int, int, int, int, int, int> calc_balance(vector<vector<int>> &board, vector<pii> &stations, vector<vector<int>> &earnlist, int turn, int T, int pos_x, int pos_y, int now_per_earn, int now_money){
    int min_dist = INF;
    ...
    if (turn+min_dist+turn_for_earn > T) return {0, INF, -1, -1, -1, -1};
    int n = board.size();
...
set<T8> decide_next_station(vector<vector<int>> &board, vector<pii> &stations, vector<vector<int>> &earnlist, int turn, int T, int now_per_earn, int now_money){
    int n = board.size();
    int max_diff = 0;
    ...
    rep(x, n)rep(y, n) if (board[x][y] == 0){
        ...
        auto [earn, new_earn, tmp_near_x, tmp_near_y, tmp_per_earn, tmp_turn_for_earn] = calc_balance(board, stations, earnlist, turn, T, x, y, now_per_earn, now_money);
        int diff = new_earn-earn;
        if (diff <= 0) continue;
        int flag = able_to_construct(board, x, y, tmp_near_x, tmp_near_y);
        if (flag != 0){
            st.emplace(diff, x, y, tmp_near_x, tmp_near_y, flag, tmp_per_earn, tmp_turn_for_earn);
            if (st.size() > BeamWidth) st.erase(st.begin());
        }
    }
    return st;
}
...
set<T8> two_station(vector<vector<int>> &earnlist, int n, int K, int T){
    set<T8> st;
    rep(x1, n)rep(y1, n)rep(x2, n)rep(y2, n){
        ...
        int res_cost = (d-1)*100+2*5000;
        int res_turn = d+1;
        int per_earn = (earnlist[x1*n+y1][x2*n+y2]+earnlist[x2*n+y2][x1*n+y1]);
        st.emplace(money, x1, y1, x2, y2, res_cost, res_turn, per_earn);
        if (st.size() > BeamWidth) st.erase(st.begin());
    }
    return st;
}
...
int main(){
    ...
    vector<T3> final_ans;
    int max_money = K;
    set<PQ> que; // PQ: now_money, board, stations, ans, now_turn, per_earn, 
    {
        auto st = two_station(earnlist, n, K, T);
        for (auto [_, x1, y1, x2, y2, cost, turn, per_earn]: st){
            vector<vector<int>> board(n, vector<int>(n));
            vector<T3> ans;
            vector<pii> stations;
            int now_money = K, now_turn = 0;
            now_money -= cost;
            now_turn += turn;
            now_money += per_earn;
            board[x1][y1] = -1;
            ...
            que.emplace(now_money, board, stations, ans, now_turn, per_earn);
            if (que.size() > BeamWidth) que.erase(que.begin());
        }
    }

    while(!que.empty()){
        set<PQ> old; swap(old, que);
        while(!old.empty()){
            auto [now_money, board, stations, ans, now_turn, per_earn] = *old.begin(); old.erase(old.begin());
            while(now_turn < T){
                if (now_money > 5000){
                    auto st = decide_next_station(board, stations, earnlist, now_turn, T, per_earn, now_money);
                    if (st.size() == 0){
                    ...
                    for (auto [_, set_x, set_y, near_x, near_y, dir, new_per_earn, turn_for_earn]: st){
                        auto tmp_board = board;
                        auto tmp_stations = stations;
                        auto tmp_ans = ans;
                        auto tmp_now_money = now_money;
                        auto tmp_now_turn = now_turn;
                        auto tmp_per_earn = per_earn;
                        // if (dir == -1){
                        //     await(tmp_ans, T-tmp_now_turn);
                        //     tmp_now_money += (T-tmp_now_turn)*tmp_per_earn;
                        //     if (max_money < tmp_now_money){
                        //         max_money = tmp_now_money;
                        //         final_ans = tmp_ans;
                        //     }
                        //     continue;
                        // }
                        await(tmp_ans, turn_for_earn);
                        tmp_now_money += turn_for_earn*tmp_per_earn;
                        tmp_now_turn += turn_for_earn;
                        int d = abs(set_x-near_x)+abs(set_y-near_y);
                        tmp_now_money -= 5000+(d-1)*100;
                        tmp_now_turn += d;
                        tmp_now_money += (d-1)*tmp_per_earn;
                        construct(tmp_board, tmp_ans, tmp_stations, set_x, set_y, near_x, near_y, dir);
                        tmp_now_money += new_per_earn;
                        tmp_per_earn = new_per_earn;
                        que.emplace(tmp_now_money, tmp_board, tmp_stations, tmp_ans, tmp_now_turn, tmp_per_earn);
                        if (que.size() > BeamWidth) que.erase(que.begin());
                    }
                }
                else{
    ...
    // {
    //     auto [money, _1, _2, ans, _3, _4] = *que.rbegin();
    //     if (max_money < money){
    //         final_ans = ans;
    //     }
    // }
...
まずinitとして2駅の位置の最適解を求める。
建設のためのお金が貯まれば設置する駅の位置を全探査して、
「建設することで今後得られるお金」>「建設しないで得られるお金」&& 最短経路で橋を設置することが可能であるもののうち、利益が一番大きい位置に駅を設置する。
駅の設置後に更新するのは、[board, ans, stations] -> construct()に任せる
[now_turn, now_money, per_earn]はmain()内で更新。
ビームサーチで
持つ状態-> board, now_money, per_earn, ans, stations, now_turn 
*/