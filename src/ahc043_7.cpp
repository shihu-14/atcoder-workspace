...
/*
前半はper_moneyで貪欲、後半は、now_moneyで貪欲を行う。
この区切りを時間の揺る限り全て探索する。
unrollつき
*/
...
T7 decide_next_station(vector<vector<int>> &board, vector<pii> &stations, vector<vector<int>> &earnlist, int turn, int T, int now_per_earn, int now_money, int donyoku_kind){
    int n = board.size();
    int max_diff = 0;
...
void rollback(vector<vector<int>> &board, vector<T3> &ans, vector<pii> &stations, int sta_x, int sta_y, int near_x, int near_y, int dir,
    int &now_money, int &now_turn, int &per_earn, int old_per_earn){

    int d = abs(sta_x-near_x)+abs(sta_y-near_y);
    now_money += 5000+(d-1)*100;
    now_turn -= d;
    now_money -= (d-1)*old_per_earn;
    now_money += per_earn;
    per_earn = old_per_earn;
    ...
    if (dir == 1){
        while(sta_x != near_x){
            sta_x++;
            if (sta_x != near_x) ans.pop_back();
            else{
                if (sta_y == near_y) break;
                if (sta_y < near_y) ans.pop_back();
                else ans.pop_back();
            }
            board[sta_x][sta_y] = 0;
        }
...
void unwait(vector<T3> &ans, int &now_money, int &now_turn, int turn, int per_earn){
    now_money -= turn*per_earn;
    now_turn -= turn;
    rep(t, turn) ans.pop_back();
}

T4 two_station(vector<vector<int>> &earnlist, int n, int K, int T){
    int max_money = K, max_x1 = -1, max_y1 = -1, max_x2 = -1, max_y2 = -1;
    ...
    rep(x1, n)rep(y1, n)rep(x2, n)rep(y2, n){
        ...
        if (money > max_money){
            max_money = money;
            max_x1 = x1, max_y1 = y1, max_x2 = x2, max_y2 = y2;
        }
    }
    return {max_x1, max_y1, max_x2, max_y2};
}

pair<vector<T3>, int> greedy(vector<vector<int>> &earnlist, vector<vector<int>> board, vector<T3> ans, vector<pii> stations, 
            int now_money, int now_turn, int per_earn, int T){
    while(now_turn < T){
        if (now_money > 5000){
            auto [set_x, set_y, near_x, near_y, dir, new_per_earn, turn_for_earn] 
                = decide_next_station(board, stations, earnlist, now_turn, T, per_earn, now_money, 0);
            if (dir == -1){
...
int main(){
    ...
    {
        tie(init_x1, init_y1, init_x2, init_y2) = two_station(earnlist, n, K, T);
        int d = abs(init_x1-init_x2)+abs(init_y1-init_y2);
        now_money -= 5000;
        now_turn += 1;
        int new_per_earn = earnlist[init_x1*n+init_y1][init_x2*n+init_y2]+earnlist[init_x2*n+init_y2][init_x1*n+init_y1];
        board[init_x1][init_y1] = -1;
        ans.emplace_back(0, init_x1, init_y1);
        stations.emplace_back(init_x1, init_y1);
        construct(board, ans, stations, init_x2, init_y2, init_x1, init_y1, 1,
            now_money, now_turn, per_earn, new_per_earn);
        history.emplace_back(init_x2, init_y2, init_x1, init_y1, 1,
                now_turn, new_per_earn);
        tie(final_ans, max_money) = greedy(earnlist, board, ans, stations, now_money, now_turn, per_earn, T);
    }
    ...
    while(now_turn < T){
        if (now_money > 5000){
            auto [set_x, set_y, near_x, near_y, dir, new_per_earn, turn_for_earn] 
                = decide_next_station(board, stations, earnlist, now_turn, T, per_earn, now_money, 1);
            if (dir == -1){
                await(ans, now_money, now_turn, T-now_turn, per_earn);
                break;
            }
            ...
            construct(board, ans, stations, set_x, set_y, near_x, near_y, dir,
                now_money, now_turn, per_earn, new_per_earn);
            history.emplace_back(set_x, set_y, near_x, near_y, dir, 
                now_turn, new_per_earn);
        }
        else{
            int tmp_q = (5000-now_money+per_earn)/per_earn;
    ...
    rrep2(i, n-2, 0){
        ...
        int dT = now_turn-pre_turn;
        unwait(ans, now_money, now_turn, dT, per_earn);
        rollback(board, ans, stations, set_x, set_y, near_x, near_y, dir,
            now_money, pre_turn, per_earn, old_per_earn);
        auto [tmp_ans, tmp_money] = greedy(earnlist, board, ans, stations, now_money, now_turn, per_earn, T);
        if (max_money < tmp_money){
            max_money = tmp_money;
            final_ans = tmp_ans;
            update++;
        }
