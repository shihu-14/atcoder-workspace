...
void build_initial_solution(int mode) {
    ...
    if (mode == 0 || mode == 3) {
        sort(rng(pairs_by_val), greater<pair<int, int>>()); 
    } else if (mode == 2 || mode == 4 || mode == 5) {
        sort(rng(pairs_by_val)); 
    } else {
        shuffle(rng(pairs_by_val), engine); 
    }
...
void hill_climbing(chrono::steady_clock::time_point start_time) {
    ...
    while (true) {
        ...
        // Swap or 2-opt (Reverse) Siblings
        else { 
