...
struct Board{
    int n, m, res_ball, put_ball, width = 1; // res_ball:まだpullしていないボールの数, put_ball:すでにpull->putしているボールの数
    vector<vector<bool>> start, goal, pres, preg;
    vector<pii> query;
    map<pii, pii> to;
    map<pii, int> id;
    Board() {}
...
struct Tree{
    ...
    void move(int t){
        ...
        history.emplace_back(string(1, dc[t]));
    }
    void rotate(int c){
    ...
    void pull(){
        ...
        rep2(i, 1, tn){
            cerr << "__¥¥ " << " " << i << endl;
             // ノードの位置が盤面に収まっていて、今ボールを掴んでいないかつ、そこにボールがある場合、pullする。
    ...
    void reset(){
        tree = pret;
        res_stk.clear();
        rep(i, board->m) res_stk.emplace_back(i);
        pulling_stk.clear();
        pulling_ball = 0;
        history.clear();
    }
...
int main(){
    ...
    auto f = [&](int times) -> bool{

    auto now =  std::chrono::high_resolution_clock::now();
        auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(now - start).count();
        return elapsed > 2000 || times >= max_times;
    };

    vector<string> ans;
    int min_times = INF, cnt = 1;
    for (int w = 1; w <= n/2; w++){
        board.width = w;
        int times = 0;
        if (f(times)) break;
        while(!tree.isfinish()){
            // cerr << "!1" << endl;
            while(!tree.toput(V-1) && !tree.res_stk.empty()){
        ...
        cerr << cnt << "回目 ->"<< times << endl;
        board.reset();
        tree.reset();
        cnt++;
        cerr << "segkakuninn2" << endl;
    }

    if (ans.size() == 0) ans = tree.history;
