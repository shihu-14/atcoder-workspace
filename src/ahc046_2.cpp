...
vector<pii> move_to(int &sx, int &sy, int gx, int gy){
    vector<pii> res;
    while(sx != gx){
        if (sx < gx){
            if (sx+1 == gx && sy == gy){
...
int main(){
    ...
    vector<pii> final_ans;
    {
        ...
        final_ans = calc_score(sx, sy, blocks, slide_list);
    ...
    while(1){
        ...
        int tmp_sx = sx, tmp_sy = sy;
        {
            for (auto [x, y]: block_list){
                auto res = move_to(tmp_sx, tmp_sy, x, y);
                final_ans.insert(final_ans.end(), res.begin(), res.end());
            }
        }
        auto slide_list = make_slide_list(blocks);
        auto tmp_ans = calc_score(tmp_sx, tmp_sy, blocks, slide_list);
