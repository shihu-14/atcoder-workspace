...
void solve()
{
    ...
    while (used_cnt < n)
    {
        ...
        if (best_score > current_threshold)
        {
            ...
            if (k == need)
            {
                int rem = h[best_b] - (k - 1) * A;
                int rep_w = best_w, rep_A = A;
                if (rem == 1)
                {
                    rep_w = -1;
                    rep_A = 1;
                ...
                {
                    int cand_w = -2, cand_A = INT_MAX;
                    rep(w2, n)
                    {
                        if (!used[w2]) continue;
                        if (c[w2] == 0) continue;
                        int A2 = a[w2][best_b];
                        if (A2 >= rem && A2 < cand_A) { cand_A = A2; cand_w = (int)w2; }
                    }
                    if (cand_w != -2 && cand_A < A)
                    {
                        rep_w = cand_w;
                        rep_A = cand_A;
                    }
                }
                rep(t, (ll)(k - 1)) ans.emplace_back(best_w, best_b);
                h[best_b] -= (k - 1) * A;
                c[best_w] -= (k - 1);
                ans.emplace_back(rep_w, best_b);
                if (rep_w != -1) c[rep_w]--;
                h[best_b] -= rep_A;
            }
            else
        ...
        {
            int rem = h[best_b];
            int rep_w = best_w, rep_A = a[best_w][best_b];
            if (rem == 1)
            {
                rep_w = -1;
                rep_A = 1;
