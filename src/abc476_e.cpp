...
{
    int n, m; cin >> n >> m;
    segtree<pii, op1, e1> seg1(n);
    segtree<pii, op2, e2> seg2(n);
    rep(i, n)
    {
        int p; cin >> p; p--;
        seg1.set(i, {p, i});
    ...
    // rep(i, n) cout << seg1.get(i).fi+1 << ' ';
    // cout << '\n';
    rep(i, m)
    {
        int l, r; cin >> l >> r; l--;
        auto [min_v, min_i] = seg1.prod(l, r);
        auto [max_v, max_i] = seg2.prod(l, r);
        seg1.set(max_i, {min_v, max_i});
        seg1.set(min_i, {max_v, min_i});
        seg2.set(max_i, {min_v, max_i});
        seg2.set(min_i, {max_v, min_i});
        // rep(i, n) cout << seg1.get(i).fi+1 << ' ';
        // cout << '\n';
    }
    rep(i, n) cout << seg1.get(i).fi+1 << ' ';
