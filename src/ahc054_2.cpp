...
// double t_g_around = 4541; // ゴールの周りの堅固さ
// double t_g_curve = 160.413; // ゴールの周りの半径t_dirの堅固さ
// double t_g_spread = 0.0; // ゴールの周りの半径t_dirの堅固さ
ll t_g_dir = 5; // ゴール周りの堅固さを評価する際の範囲
ll t_g_straight = 10; // ゴール周りの堅固さを評価する際の直線距離
        ...
        for (int d2: {0, 2})
        {
                ...
                int d1, int d2, bool ok, int curve, int straight) -> void
            {
                ...
                if (curve >= dir) 
                {
                    int r = reachable(stk);
                    if (r == INF) return;
                    if (chmax(score, {curve, r, -straight}))
                    {
                        res = stk;
                    }
                    return;
