...
vector<pii> two_station(vector<vector<int>> &earnlist, int n, int K, int T){
    ...
    vector<pii> init_list = {{max_p1, max_p2}}, init_list2, init_list3;
    {
        rep(x1, n)rep(y1, n)rep(x2, n)rep(y2, n){
            int d = abs(x1-x2)+abs(y1-y2);
            if (d <= 4) continue;
...
int main(){
    ...
    for (auto [p1, p2]: init_list){
        ...
        if (elapsed.count() > 3000) break;
        cerr << times << " " << elapsed.count() << endl;
        while(now_turn < T){
            money_history.emplace_back(now_turn, now_money);
            if (now_turn >= T){
                int it = lower_bound(rng(max_money_history), make_pair(now_turn, now_money))-max_money_history.begin();
                if (it > 0 && max_money_history[it-1].se > now_money){
                    miss++;
                    break;
                }
            }
        ...
        if (max_money < now_money){
            max_money = now_money;
            final_ans = ans;
            max_money_history = money_history;
            update++;
        }
