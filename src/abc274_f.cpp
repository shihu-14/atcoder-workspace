...
struct Event{
    ...
    ll u, v; 
    int id;
    Event(frac _t, ll _u, ll _v, int _id): t(_t), u(_u), v(_v), id(_id) {}
...
struct Fish{
    ll w, x, v; 
    Fish(): w(0), x(0), v(0) {}
    Fish(ll _w, ll _x, ll _v): w(_w), x(_x), v(_v) {}
...
int main(){
    ...
    int n, a; cin >> n >> a;
    vector<Fish> fish(n);
    rep(i, n) cin >> fish[i].w >> fish[i].x >> fish[i].v;
    sort(rng(fish));
    vector<Event> event;
    rep(i, n)rep(j, n){
        if (i == j) continue;
        if (fish[i].v == fish[j].v) continue;
        else{
            frac t = frac(a-fish[j].x+fish[i].x, fish[j].v-fish[i].v);
            if (t <= 0) continue;
            event.emplace_back(t, i, j, 0);
        }
    }
    rep(i, n)rep(j, n){
        if (fish[j].x < fish[i].x && fish[j].v > fish[i].v) event.emplace_back(frac(fish[i].x-fish[j].x, fish[j].v-fish[i].v), i, j, 1);
    }
    sort(rng(event));
    vector<ll> ord(n), s(n+1);
    ll ans = 0;
    rep(i, n) ord[i] = i+1;
    rep(i, n){
        s[i+1] = s[i] + fish[i].w;
    ...
    rep(l, n){
        int r = upper_bound(rng(fish), fish[l].x + a, [](ll val, const Fish& f) {
            return val < f.x;
        }) - fish.begin();
        
        // int r = upper_bound(rng(fish), {fish[l].w, fish[l].x+a, fish[l].v}) - fish.begin();
        chmax(ans, s[r]-s[l]);
    }
    rep(i, event.size()){
        auto [t, u, v, id] = event[i];
        if (id){
            s[ord[v]] = s[ord[v]]-fish[v].w+fish[u].w;
            swap(ord[u], ord[v]);
        }
        else{
            chmax(ans, s[ord[v]]-s[ord[u]-1]);
        }
    }
    cout << ans << endl;
    return 0;
