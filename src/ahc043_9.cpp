...
pair<T4, vector<pii>> two_station(vector<vector<int>> &earnlist, int n, int K, int T){
    int max_money = K, max_x1 = -1, max_y1 = -1, max_x2 = -1, max_y2 = -1;
    int res_cost = -1, res_turn = -1, per_earn = -1;
    vector<pii> init_list, init_list2;
    rep(x1, n)rep(y1, n)rep(x2, n)rep(y2, n){
        ...
        if (money > 3*max_money/4){
            init_list.emplace_back(x1*n+y1, x2*n+y2);
        }
        else if (money > 2*max_money/3){
            init_list2.emplace_back(x1*n+y1, x2*n+y2);
        }
    }
    init_list.insert(init_list.end(), init_list2.begin(), init_list2.end());
    return {{max_x1, max_y1, max_x2, max_y2}, init_list};
}
...
int main(){
    ...
    {
        auto [mx, cand] = two_station(earnlist, n, K, T);
