    ...
    rep2(i, 1, K)
    {
        ...
        }
        {
            ...
            while(x != sx || y != sy)
            {
                ...
                if (px == sx && py == sy && i-2 >= 0) qv.emplace_back(px*n+py, i-2, px*n+py, i-1, dc[d]);
                else qv.emplace_back(px*n+py, i-1, px*n+py, i-1, dc[d]);
                x = px, y = py;
            }
        }
        sx = gx, sy = gy;
    }
    cout << n*n << " " << K << " " << qv.size() << endl;
    rep(i, n)
    {
        rep(j, n) cout << i*n + j << " ";
        cout << endl;
