...
int main(){
    ...
    rep(i, n) que[i] = pa[i].se;
    std::random_device rd; 
    std::mt19937 gen(rd());
    std::uniform_int_distribution<> dist(0, n-1), dist_half(0, n/2);
    int times = 0, update = 0;
    // 
    while(1){
        auto end = std::chrono::high_resolution_clock::now();
        auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
        if (elapsed.count() > 1985) break;
        times++;
        // tmp
        int tmp_score = 0;
        // int tmp_start = dist(gen);
        vector<int> tmp_que = que;
        G tmp_g = g;
        rep(t, 10){
            int j1 = dist_half(gen);
            int j2 = dist_half(gen);
