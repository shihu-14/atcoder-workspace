...
void solve()
{
    int n; cin >> n;
    vector<int> p(n);
    rep2(i, 1, n) cin >> p[i], p[i]--;
    vector<vector<int>> g(n);
    rep2(i, 1, n) g[p[i]].emplace_back(i);
    ...
    auto dfs = [&](auto f, int v, int p=-1) -> void
    {
        ...
        // cout << v << ": "     << sum_c[v]-d_sum << " " << D[v] << endl;
        ans *= comb(sum_c[v]-d_sum, D[v]);
    };
    dfs(dfs, 0);
    cout << ans.val() << endl;
}
