...
struct Tree{
    ...
    pii find_center(){
        ...
        vector<bool> used(n);
        queue<int> q;
        rep(i, n) if (tmp_deg[i] == 1) q.emplace(i);
        while(res_n > 2){
            res_n -= q.size();
            queue<int> tmp_q;
            while(!q.empty()){
...
int main(){
    ...
    auto [u, v] = t.find_center();
    vector<int> center;
    if (u != -1) center.emplace_back(u);
    if (v != -1) center.emplace_back(v);

    int ans = INF;
    for (auto u: center){
        vector<int> deg;
        for (auto v: g[u]){
            deg.emplace_back(g[v].size()-1);
        }
        sort(rrng(deg));
        rep(i, g[u].size()){
            int res = (i+1)+1;
