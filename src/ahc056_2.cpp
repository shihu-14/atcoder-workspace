    ...
    vector<T5> qv;
    {
        ...
        rep2(i, 1, K)
        {
            ...
            while(x != gx || y != gy)
            {
                int d = g[x][y][i-1];
                int nx = x + dx[d], ny = y + dy[d];
                if (x == sx && y == sy && i-2 >= 0) qv.emplace_back(col[x][y], i-2, col[x][y], i-1, dc[d]);
                else qv.emplace_back(col[x][y], i-1, col[x][y], i-1, dc[d]);
                x = nx, y = ny;
            }
        }
    }
    UNIQUE(qv);
    cout << C << " " << K-1 << " " << qv.size() << endl;
    rep(i, n)
    {
        rep(j, n) cout << col[i][j] << " ";
        cout << endl;
    }
