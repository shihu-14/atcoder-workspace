...
int main(){
    ...
    rep(w, n){
        rep(l, n){
            ...
            if (l && r < n){
                // cout << l-1 << " " << r+1 << endl;
                if (s[l-1] == '?' && s[r] == '?') dp2[l-1][r+1] += 26*dp2[l][r];
    ...
    printvv(dp2);
    mint ans;
    vector<mint> c(n+1), a(n+1, 1);
    rep(i, n) c[i+1] = s[i] == '?';
    rep(i, n) c[i+1] += c[i];
    rep(i, n) a[i+1] *= a[i]*26;
    rep(i, n){
        rep2(j, i+1, n+1){
            ans += a[(c[n]-(c[j]-c[i])).val()]*dp2[i][j];
        }
    }
    cout << ans.val() << endl;
    // vector<mint> dp(n+1);
    // rep(i, n){
    //     dp[i+1] += dp[i]*(s[i] == '?' ? 26 : 1);
    //     rep(j, i+1){
    //         dp[i+1] += dp2[j][i+1];
    //     }
    // }
