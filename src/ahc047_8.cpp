...
double es(array<double, A> &p, vector<P> &v){
    double r = 0;
    for (auto &t: v){
        double lg = 0;
...
void bm(array<double, A> &p, array<array<int, M>, M> &a){
    array<double, M> raw;
    ...
    int r = 100-s;
    sort(rng(fr), [](auto x, auto y) { return x.fi > y.fi; });
    rep(i, r) row[fr[i].se]++;
...
int main(){
    ...
    while(1){
        if (chrono::duration<double>(chrono::steady_clock::now() - st).count() > 1.9) break; 
        times++;
        auto cp = bp;
        int x = rg()%A, y;
        do{
