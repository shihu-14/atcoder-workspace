...
/*
two_stationsから得られる初期配置をnow_moneyが大きい順に時間が許す限り全て試す。
...
T6 calc_balance(vector<vector<int>> &board, vector<pii> &stations, vector<vector<int>> &earnlist, int turn, int T, 
...
T7 decide_next_station(vector<vector<int>> &board, vector<pii> &stations, vector<vector<int>> &earnlist, int turn, int T, int now_per_earn, int now_money){
    int n = board.size();
    int max_diff = 0;
...
vector<pii> two_station(vector<vector<int>> &earnlist, int n, int K, int T){
    ...
    vector<pii> init_list, init_list2, init_list3;
    init_list.emplace_back(max_x1*n+max_y1, max_x2*n+max_y2);
    ...
    rep(x1, n)rep(y1, n)rep(x2, n)rep(y2, n){
        ...
        if (money < 0) continue;
        money += (earnlist[x1*n+y1][x2*n+y2]+earnlist[x2*n+y2][x1*n+y1])*(T-turn);
        if (money > 2*max_money/3){
            init_list.emplace_back(x1*n+y1, x2*n+y2);
        }
        // else if (money > 2*max_money/3){
        //     init_list2.emplace_back(x1*n+y1, x2*n+y2);
        // }
        // else if (money > max_money/3){
        //     init_list3.emplace_back(x1*n+y1, x2*n+y2);
        // }
        // if (init_list.size() > 50) return init_list;
    }
    // init_list.insert(init_list.end(), init_list2.begin(), init_list2.end());
    // if (init_list.size() > 50) return init_list;
    // init_list.insert(init_list.end(), init_list3.begin(), init_list3.end());
    return init_list;
}
...
int main(){
    ...
    auto init_list = two_station(earnlist, n, K, T);
    ...
    for (auto [p1, p2]: init_list){
        ...
        while(now_turn < T){
            ...
            // }
            if (now_money > 5000){
                auto [set_x, set_y, near_x, near_y, dir, new_per_earn, turn_for_earn] = decide_next_station(board, stations, earnlist, now_turn, T, per_earn, now_money);
                if (dir == -1){
        ...
        if (max_money < now_money){
            ...
            final_ans = ans;
            // if (update == 0){
            //     cerr << "pre-performance: " << now_money << "-> ";
            // }
            // update++;
        }
