...
// =============================
// Tunable parameters (loaded from file)
// =============================
struct TunableParams {
    // Time / search
    double time_limit = 1.94;   // was TIME_LIMIT
    double ucb_c      = 0.35;   // was C in UCB1
    ...
    // Base AI params for players (fallback init)
    double base_wa = 0.6;
    double base_wb = 0.6;
    double base_wc = 0.6;
    double base_wd = 0.6;
    double base_eps = 0.25;

    // choose_my_rollout_move weights
    double my_empty_mult = 3.0;

    double my_own_ltU_threat_mult = 6.0;
    double my_own_ltU_safe_mult   = 0.6;

    double my_own_eqU_threat_mult = 0.8;
    double my_own_eqU_safe_mult   = 0.05;

    double my_enemy_lvl1_leader_mult = 10.0;
    double my_enemy_lvl1_other_mult  = 6.0;

    double my_enemy_lvlge2_leader_mult = 1.2;
    double my_enemy_lvlge2_other_mult  = 0.8;

    double my_min_w = 0.01;

    // fast_eval_candidate weights
    double fast_empty_mult = 3.0;
    double fast_own_ltU_mult = 1.0;
    double fast_own_eqU_mult = 0.05;

    double fast_enemy_lvl1_leader_mult = 10.0;
    double fast_enemy_lvl1_other_mult  = 6.0;

    double fast_enemy_lvlge2_leader_mult = 0.9;
    double fast_enemy_lvlge2_other_mult  = 0.6;
};

static TunableParams G; // global params
...
static inline bool parse_double(const string &s, double &out) {
    char *endp = nullptr;
...
static inline bool parse_int(const string &s, int &out) {
    char *endp = nullptr;
...
static bool load_params_file(const string &path, TunableParams &p) {
    ...
    while (getline(fin, line)) {
        ...
        // Time / search
        if (key == "time_limit" && parse_double(val, dv)) p.time_limit = dv;
        else if (key == "ucb_c" && parse_double(val, dv)) p.ucb_c = dv;

        // Depth schedule
        else if (key == "t_early" && parse_int(val, iv)) p.t_early = iv;
        else if (key == "t_mid" && parse_int(val, iv)) p.t_mid = iv;
        else if (key == "depth_early" && parse_int(val, iv)) p.depth_early = iv;
        else if (key == "depth_mid" && parse_int(val, iv)) p.depth_mid = iv;
        else if (key == "depth_late" && parse_int(val, iv)) p.depth_late = iv;

        // Base AI params
        else if (key == "base_wa" && parse_double(val, dv)) p.base_wa = dv;
        else if (key == "base_wb" && parse_double(val, dv)) p.base_wb = dv;
        else if (key == "base_wc" && parse_double(val, dv)) p.base_wc = dv;
        else if (key == "base_wd" && parse_double(val, dv)) p.base_wd = dv;
        else if (key == "base_eps" && parse_double(val, dv)) p.base_eps = dv;

        // choose_my_rollout_move weights
        else if (key == "my_empty_mult" && parse_double(val, dv)) p.my_empty_mult = dv;

        else if (key == "my_own_ltU_threat_mult" && parse_double(val, dv)) p.my_own_ltU_threat_mult = dv;
        else if (key == "my_own_ltU_safe_mult" && parse_double(val, dv)) p.my_own_ltU_safe_mult = dv;

        else if (key == "my_own_eqU_threat_mult" && parse_double(val, dv)) p.my_own_eqU_threat_mult = dv;
        else if (key == "my_own_eqU_safe_mult" && parse_double(val, dv)) p.my_own_eqU_safe_mult = dv;

        else if (key == "my_enemy_lvl1_leader_mult" && parse_double(val, dv)) p.my_enemy_lvl1_leader_mult = dv;
        else if (key == "my_enemy_lvl1_other_mult" && parse_double(val, dv)) p.my_enemy_lvl1_other_mult = dv;

        else if (key == "my_enemy_lvlge2_leader_mult" && parse_double(val, dv)) p.my_enemy_lvlge2_leader_mult = dv;
        else if (key == "my_enemy_lvlge2_other_mult" && parse_double(val, dv)) p.my_enemy_lvlge2_other_mult = dv;

        else if (key == "my_min_w" && parse_double(val, dv)) p.my_min_w = dv;

        // fast_eval_candidate weights
        else if (key == "fast_empty_mult" && parse_double(val, dv)) p.fast_empty_mult = dv;
        else if (key == "fast_own_ltU_mult" && parse_double(val, dv)) p.fast_own_ltU_mult = dv;
        else if (key == "fast_own_eqU_mult" && parse_double(val, dv)) p.fast_own_eqU_mult = dv;

        else if (key == "fast_enemy_lvl1_leader_mult" && parse_double(val, dv)) p.fast_enemy_lvl1_leader_mult = dv;
        else if (key == "fast_enemy_lvl1_other_mult" && parse_double(val, dv)) p.fast_enemy_lvl1_other_mult = dv;

        else if (key == "fast_enemy_lvlge2_leader_mult" && parse_double(val, dv)) p.fast_enemy_lvlge2_leader_mult = dv;
        else if (key == "fast_enemy_lvlge2_other_mult" && parse_double(val, dv)) p.fast_enemy_lvlge2_other_mult = dv;
...
static inline Move choose_my_rollout_move(const State &s, int leader, Xorshift &rng, vector<Move> &tmp) {
    ...
    for (int i=0; i<(int)tmp.size(); ++i) {
        ...
        if (o == -1) {
            ...
            if (s.level[r][c] < U) w *= threatened ? G.my_own_ltU_threat_mult : G.my_own_ltU_safe_mult;
            else w *= threatened ? G.my_own_eqU_threat_mult : G.my_own_eqU_safe_mult;
        } else {
            if (s.level[r][c] == 1) {
                w *= (o == leader ? G.my_enemy_lvl1_leader_mult : G.my_enemy_lvl1_other_mult);
            } else {
                w *= (o == leader ? G.my_enemy_lvlge2_leader_mult : G.my_enemy_lvlge2_other_mult);
            }
        }

        if (w < G.my_min_w) w = G.my_min_w;
        weights[i] = w;
        sum += w;
    }
...
static inline double fast_eval_candidate(const State &s, const Move &m, int leader) {
    ...
    int o = s.owner[r][c];
    if (o == -1) return base * G.fast_empty_mult;
    if (o == 0) {
        if (s.level[r][c] < U) return base * G.fast_own_ltU_mult;
        return base * G.fast_own_eqU_mult;
    }
    if (s.level[r][c] == 1) return base * (o == leader ? G.fast_enemy_lvl1_leader_mult : G.fast_enemy_lvl1_other_mult);
    return base * (o == leader ? G.fast_enemy_lvlge2_leader_mult : G.fast_enemy_lvlge2_other_mult);
}
...
int main(int argc, char **argv) {
    ...
    if (argc >= 2) {
        (void)load_params_file(argv[1], G);
    }
    ...
    for (int t=0; t<T; ++t) {
        ...
        double remain = G.time_limit - cur_time;
        ...
        for (int p=0; p<M; ++p) ai_params[p] = {G.base_wa, G.base_wb, G.base_wc, G.base_wd, G.base_eps};
        ...
        int depth;
        if (t < G.t_early) depth = G.depth_early;
        else if (t < G.t_mid) depth = G.depth_mid;
        else depth = G.depth_late;
