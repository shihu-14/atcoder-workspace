...
struct AIModel {
    ...
    AIParam sample_param(Xorshift &rng) const {
        double r = rng.next_double();
        double acc = 0.0;
        for (int i=0; i<P; ++i) {
            acc += ps[i].w;
            if (r <= acc) return ps[i].par;
        }
        return ps[P-1].par;
    }
...
static inline Move choose_my_rollout_move(const State &s, int leader, Xorshift &rng, vector<Move> &tmp) {
    ...
    for (int i=0; i<(int)tmp.size(); ++i) {
        ...
        if (o == -1) {
            ...
            if (s.level[r][c] < U) w *= threatened ? 6.0 : 0.6;
            else w *= threatened ? 0.8 : 0.05;
        } else {
            if (s.level[r][c] == 1) {
                w *= (o == leader ? 10.0 : 6.0);
            } else {
...
// -----------------------------
// Candidate pruning heuristic (improved)
// -----------------------------
static inline double fast_eval_candidate(const State &s, const Move &m, int leader) {
    ...
    if (o == 0) {
        ...
        if (s.level[r][c] < U) return base * (threatened ? 6.0 : 1.0);
        return base * (threatened ? 0.3 : 0.05);
    }
...
int main() {
    ...
    for (int t=0; t<T; ++t) {
        ...
        int Kkeep = min((int)scored.size(), MAX_CAND);
        nth_element(scored.begin(), scored.begin() + Kkeep - 1, scored.end(),
        ...
        // common seed stream per turn (variance reduction)
        ...
        for (int r=0; r<N; ++r) for (int c=0; c<N; ++c) base = base * 1315423911ULL + (uint64_t)(turn_start.owner[r][c] + 2);

        uint64_t seed_stream = base;

        int total = 0;
        for (int i=0; i<K; ++i) {
            uint64_t sseed = splitmix64(seed_stream);
            double sc = run_simulation(turn_start, top[i], depth, ai_models, sseed);
            sum[i] += sc;
            cnt[i]++;
            total++;
        }
        ...
        while (true) {
            ...
            uint64_t sseed = splitmix64(seed_stream);
            double sc = run_simulation(turn_start, top[best_i], depth, ai_models, sseed);
            sum[best_i] += sc;
            cnt[best_i]++;
            total++;
            loop++;
